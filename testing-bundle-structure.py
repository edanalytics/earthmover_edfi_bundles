...
# TODO: defining the tests

FILES = "Bundle Folder Contents"
PARAMS = "Getting EM Parameters"
COMPL = "Compiling earthmover.yaml"
IDPAC = "ID Package Compatibility"


# TODO: exceptions for the testing 
...
class Checker:
    def __init__(self, folder_path):
        ...
        self.all_tests = [FILES, PARAMS, COMPL, IDPAC]

        self.warnings = {test: [] for test in self.all_tests}
        self.successes = {test: [] for test in self.all_tests}

        self.not_run_tests = list(self.not_run_tests)
    ...
    def checking_folder_contents(self):
        ...
        if set(required_contents) - set(folder_contents) == set():
            self.successes[FILES].append("You seem to have all the necessary files and folders at the root.\n          "+ 
                                                            "\n          ".join(f"{item}" for item in required_contents)+"\n")
        else:
            missing_contents = list(set(required_contents) - set(folder_contents))
            self.warnings[FILES].append(f"You are missing the following files/folders:\n          "+
                                                           "\n          ".join(f"{item}" for item in missing_contents)+"\n")  

        n_em_yaml = sum("earthmover" in f and f.endswith(".yaml") for f in folder_contents)
        n_lb_yaml = sum("lightbeam" in f and f.endswith(".yaml") for f in folder_contents)

        if n_em_yaml > 1:
            self.warnings[FILES].append(f"Please make sure there is one earthmover config file in your bundle, found {n_em_yaml}")
        else:
            self.successes[FILES].append("There is a single earthmover config file")

        if n_lb_yaml > 1:
            self.warnings[FILES].append(f"Please make sure there is one lightbeam config file in your bundle, found {n_lb_yaml}")
        else:
            self.successes[FILES].append("There is a single lightbeam config file")
        ...
        if data_folder:
            ...
            self.successes[FILES].append("Found the following sample data files:\n          " +
                                                            "\n          ".join(file for file in data_folder))
        
        if not self.has_sample_data:
            self.warnings[FILES].append("Missing a sample data file")


        self.not_run_tests.remove(FILES)
    ...
    def getting_params(self):
        ...
        for pattern in ["./earthmover.yaml", "earthmover.yaml"]:
            try:
                ...
                self.successes[PARAMS].append("Found sample Earthmover run command in the README.md")
                break
            except IndexError:
                continue
        else:
            self.warnings[PARAMS].append("Please make sure there is a sample Earthmover run command in README.md")
        ...
        try:
            ...
            self.successes[PARAMS].append("The Earthmover command is valid JSON")
        except json.JSONDecodeError:
            self.warnings[PARAMS].append("Unable to use the provided earthmover command, invalid JSON: ", params_string) 

        self.not_run_tests.remove(PARAMS)
    ...
    def compiling_yaml(self):
        ...
        if result.returncode == 0:
            self.successes[COMPL].append("Successfully compiled with Earthmover")
        else:
            self.warnings[COMPL].append(f"Error during compiling with Earthmover:\n{result.stdout}\n{result.stderr}")

        self.not_run_tests.remove(COMPL)
    ...
    def checking_id_package_compatibility(self):
        ...
        try:
            if self.compiled_em_yaml_text["transformations"]["input"] == {'source': '$sources.input', 'operations': []}:
                self.successes[IDPAC].append("Correct transformations.input")
            else:
                raise InvalidBlockError
        except (KeyError, InvalidBlockError):
            self.warnings[IDPAC].append("Please make sure the following block is in your `earthmover.yaml`:\n\n"
                                                                "transformations:\n"
                                                                "  input:\n"
        ...
        try:
            if self.compiled_em_yaml_text["config"]["parameter_defaults"]["STUDENT_ID_NAME"] == "edFi_studentUniqueID":
                self.successes[IDPAC].append("Correct config.parameter_defaults.STUDENT_ID_NAME")
            else:
                raise InvalidBlockError
        except (KeyError, InvalidBlockError):
            self.warnings[IDPAC].append("Please make sure the following block is in your `earthmover.yaml`:\n\n"
        ...
        self.not_run_tests.remove(IDPAC)
...
"""
    # TODO: this code parses the studentAssessment(s) file, doesn't # FIXME: say more
